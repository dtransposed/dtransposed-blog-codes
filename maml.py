...
class MAML:
    ...
    def derivative_loss(self, x, y, params):
    ...
    def plot(self, batch_of_tasks):
        ...
        for i, a in enumerate(self.plot_history):
            ...
            ax.scatter(xs=[a[0]],
                    ys=[a[1]],
                    zs=[self.loss(self.x_val, self.y_val, a)], c='black',
                    s=50,
                    alpha = float(i/4))

        for i, task in enumerate(batch_of_tasks):
            ax.scatter(xs=[task[0]], ys=[task[1]], zs=[self.loss(self.x_val, self.y_val, self.goal_parameters)],
                       marker = "*", c='purple', s=50)
    ...
    def run(self):
        ...
        while True:
            ...
            if np.linalg.norm(self.parameters-self.goal_parameters) < 1:
                break
            self.plot(batch_of_tasks)
            print(self.parameters)
            print(self.goal_parameters)
